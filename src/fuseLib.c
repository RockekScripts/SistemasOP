 ...
 *
*/
static int my_unlink(const char* fName){

	char* name = (char*)fName + 1;


	int index = 0;
	index = findFileByName(&myFileSystem, name);
	...
	int posNode = myFileSystem.directory.files[index].nodeIdx;



if(myFileSystem.nodes[posNode]->nlinks == 1){

	myFileSystem.numFreeNodes += 1;
	int i = 0;
	for(i = 0; i < myFileSystem.nodes[posNode]->numBlocks; i++){
		myFileSystem.bitMap[myFileSystem.nodes[posNode]->blocks[i]] = 0;

	}

	myFileSystem.superBlock.numOfFreeBlocks += myFileSystem.nodes[posNode]->numBlocks;
	updateBitmap(&myFileSystem);
	updateSuperBlock(&myFileSystem);
	myFileSystem.nodes[posNode]->fileSize = 0;
	myFileSystem.nodes[posNode]->freeNode = true;
	myFileSystem.nodes[posNode]->numBlocks = 0;
	myFileSystem.nodes[posNode]->nlinks--;

}
else {
	myFileSystem.nodes[posNode]->nlinks--;
}
	updateNode(&myFileSystem, posNode, myFileSystem.nodes[posNode]);
...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info * ffi){
	int  i = 0;
	char buffer[BLOCK_SIZE_BYTES]; //bloque de caracteres
	int bytesLeer = size;
	int bytes2Read = 0;
	NodeStruct* node = myFileSystem.nodes[ffi->fh];

	while (bytesLeer > 0){
		int bloqActual = node->blocks[offset / BLOCK_SIZE_BYTES];
		int offblock = offset % BLOCK_SIZE_BYTES;

//mueves en el disco virtual al principio del bloque y te mueves bloqActual * BLOCK_SIZE_BYTES bytes

		if((lseek(myFileSystem.fdVirtualDisk, bloqActual * BLOCK_SIZE_BYTES, SEEK_SET) == (off_t) - 1)){
			return -1;
		}

		int leidos = read(myFileSystem.fdVirtualDisk, &buffer, BLOCK_SIZE_BYTES);
		if(leidos == -1){
			return EXIT_FAILURE;
		}

		//si no da error y es distinto de cero (algo que escribir), copiamos lo que nos pasan en buf
		if(leidos != 0){

			for(i = offblock; (i < BLOCK_SIZE_BYTES) && (bytes2Read < size); i++){
				buf[bytes2Read] = buffer[i];
				bytes2Read++;
			}

			bytesLeer -= (i - offblock);
			offset += i;
		}
	}
...
static int my_link (const char* from, const char* to){
	...
	if((idxDirF = findFileByName(&myFileSystem, (char *)from + 1)) == -1) {
	        return -ENOENT;
	}
	if(!(idxDirT = findFileByName(&myFileSystem, (char *)to + 1)) == -1) {
	        return -ENOENT;
	}
	...
	int posNodeF = myFileSystem.directory.files[idxDirF].nodeIdx;
	 ...
	 int  idxDir;
	    ...
	    myFileSystem.directory.files[idxDir].nodeIdx = posNodeF;
	   // myFileSystem.numFreeNodes--;




	    myFileSystem.nodes[posNodeF]->modificationTime = time(NULL);
	    myFileSystem.nodes[posNodeF]->nlinks++;


	    updateDirectory(&myFileSystem);
	    updateNode(&myFileSystem, posNodeF, myFileSystem.nodes[posNodeF]);
	//int posNodeT = myFileSystem.directory.files[idxDirT].nodeIdx;
	//myFileSystem.nodes[posNodeT]->nlinks++;
